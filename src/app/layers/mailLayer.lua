...
function mailLayer:move_layer(_layer)
    ...
    local move =cc.MoveTo:create(0.2,cc.p(_layer:getPositionX(),curr_y-50)) 
     local move1 =cc.MoveTo:create(0.2,cc.p(_layer:getPositionX(),curr_y+10))  
       local move2 =cc.MoveTo:create(0.2,cc.p(_layer:getPositionX(),curr_y))    
      --local sque=transition.sequence({cc.EaseElasticOut:create(move)})
       local sque=cc.Sequence:create(  move,move1,move2)
