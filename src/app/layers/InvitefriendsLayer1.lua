...
function InvitefriendsLayer:fun_init_infor(  )
      self.FriendsNode = cc.CSLoader:createNode("FriendsNode.csb")  --邀请好友排行榜
      self:addChild(self.FriendsNode)
      --关闭
       local sp_exit=self.FriendsNode:getChildByName("sp_exit")
       sp_exit:addTouchEventListener(function(sender, eventType  )
       ...
       --添加
       local sp_Search=self.FriendsNode:getChildByName("sp_Search")
       sp_Search:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
                         self.f_count=0
       ...
       --删除
       local sp_DeleteFriends=self.FriendsNode:getChildByName("sp_DeleteFriends")
       sp_DeleteFriends:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
      ...
      --邀请好友
       local btn_InviteFriends=self.FriendsNode:getChildByName("btn_InviteFriends")
       btn_InviteFriends:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
                        print("邀请好友")
       ...
       --领取奖励
       self.obtain_bt=self.FriendsNode:getChildByName("sp_rewards")
      ...
      local friendsCell=self.FriendsNode:getChildByName("friendsCell")
      self._ListView=friendsCell:getChildByName("pnl_list")
      self._ListView:setItemModel(self._ListView:getItem(0))
      self._ListView:removeAllItems()
      --确定
       self.addFriend_truebt=self.FriendsNode:getChildByName("sp_tFriends")
       btn_InviteFriends:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
       ...
       --取消
       self.addFriend_falsebt=self.FriendsNode:getChildByName("sp_fFriends")
       btn_InviteFriends:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
...
function InvitefriendsLayer:fun_infor(  )
      self.CarryFriendsNode = cc.CSLoader:createNode("CarryFriendsNode.csb")  --邀请好友排行榜
      self:addChild(self.CarryFriendsNode)
end
...
function InvitefriendsLayer:fun_init( _isvisber)
            ...
            self._table_box={}
             ...
             if  not friendlist_table then
              ...
              self._ListView:removeAllItems()
            self.gold_text=self.FriendsNode:getChildByName("sp_gold")  --未领取的金币
            ...
            for i=1,#_friendlist do
	          	...
	          	local  _cell =  self._ListView:getItem(i-1)
	            _cell:setTag(i)
	            self.nickname = _cell:getChildByName("txt_FriendName")   --名字
              self.nickname:setString(_friendlist[i]["nickname"])
              self.grade=_cell:getChildByName("txt_FriendHonour")  --等级
              self.grade:setString( _friendlist[i]["playergrade"] )
              self.imgurl =  _cell:getChildByName("sp_Icon") --头像
              self.imgurl:loadTexture("png/"   ..   string.lower(tostring(Util:sub_str(_friendlist[i]["imgurl"], "/",":"))))
                  ...
                  -- self.total_golds:setString( _friendlist[i]["total_points"] )
                   ...
                   local move_friend =_cell:getChildByName("sp_AddFriends")  --删除好友

                   local yao_text_friend =_cell:getChildByName("txt_FriendType")  --邀字 
