...
function FloatingLayer:showFloat(text,is_resource,resourceType)
	...
	name:setPosition(320,self.height/2+5)
	name:addTo(floating_bg,100)


	table.insert(self.float_array,floating_bg)
	--在这里需要更改
	...
	floating_bg:runAction(sequence)
end
	...
	for i= 1,self.float_number do
		...
		if self.float_number > 1 then
			if sprite:getPositionY() < display.cy + i*self.height +rect.height/2 then
				local i_down_konw = display.cy+i*self.height+rect.height/2
