...
--   测试类 
local PerInformationScene = class("PerInformationScene", function()
    return display.newScene()
end)

function PerInformationScene:ctor()
        self:setNodeEventEnabled(true)--layer添加监听
        self.filename=" "--params.filename       
        self.point=cc.p(0,0)  --params.point
        --self._size=params._size
        self.row=4--params.row
        self.col=4--params.col
        self.count=4*4--params.row* params.col

           self._originalimage = cc.CSLoader:createNode("originalimage.csb")
           self:addChild(self._originalimage)
        ...
        self.fragment_sprite_bg = self.original--display.newScale9Sprite(self.filename, self.point.x,self.point.y, cc.size(self._size.width,self._size.height))
       -- self.fragment_sprite_bg:setAnchorPoint(1,1)
        self.content_size = self.original:getContentSize()
        self._size=self.original:getContentSize()
...
function PerInformationScene:refresh_table()
   ...
   for i=1,row do
        for j=1,col do
        	   --sprite:setTextureRect(cc.rect(0, 0, 120, 50))
                local fragment_sprite =self.original:setTextureRect(cc.rect(self.content_size.width/row-3, self.content_size.height/col-3, self._size.width,self._size.height))-- display.newScale9Sprite(self.filename, 0,0, cc.size(self._size.width,self._size.height))
               ...
               -- fragment_sprite:setPosition(0 - (i-1)*self.content_size.width/row, 0 - (j-1)*self.content_size.height/col)--设置图片显示的部分
                self._originalimage:addChild(clipnode)
...
function PerInformationScene:RandomIndex(indexNum, tabNum)
...
function PerInformationScene:touch_event_move(event,clipnode)
...
function PerInformationScene:sort_sure()
...
function PerInformationScene:touchEnd(event,clipnode,pos)
...
function PerInformationScene:saw_issuccess()
