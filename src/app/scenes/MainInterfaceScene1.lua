...
function MainInterfaceScene:fun_infor(  )
              self.MainScene = cc.CSLoader:createNode("MainScene.csb")
              self:addChild(self.MainScene)
              self.MainScene_Center=self.MainScene:getChildByName("MainScene_Center")
              self.MainScene_PlayerStates=self.MainScene:getChildByName("MainScene_PlayerStates")
              self.MainScene_ToolBar=self.MainScene:getChildByName("MainScene_ToolBar")

              local pnl_Center=self.MainScene_Center:getChildByName("pnl_Center")
              self.pnl_PlayerStates=self.MainScene_PlayerStates:getChildByName("pnl_PlayerStates")

              --  头像点击事件
              local Z_TX_D=self.pnl_PlayerStates:getChildByName("Z_TX_D")
              Z_TX_D:addTouchEventListener(function(sender, eventType  )
              ...
              --金币奖池
              local btn_SurpriseScene=pnl_Center:getChildByName("btn_SurpriseScene")
              btn_SurpriseScene:addTouchEventListener(function(sender, eventType  )
                   if eventType ~= ccui.TouchEventType.ended then
                       return
                  end
              ...
              --商城
              local btn_JackpotScene=pnl_Center:getChildByName("btn_JackpotScene")
              btn_JackpotScene:addTouchEventListener(function(sender, eventType  )
                 if eventType ~= ccui.TouchEventType.ended then
                       return
                  end
                       local taskLayer = require("app.layers.taskLayer")  --邀请好友
            self:addChild(taskLayer.new(),1,11)
              ...
              --排行
              local btn_ActivityScene=pnl_Center:getChildByName("btn_ActivityScene")
              btn_ActivityScene:addTouchEventListener(function(sender, eventType  )
                 if eventType ~= ccui.TouchEventType.ended then
                       return
                  end
              ...
              -- 许愿树
              local btn_LoadCardScene=pnl_Center:getChildByName("btn_LoadCardScene")
              btn_LoadCardScene:addTouchEventListener(function(sender, eventType  )
                 if eventType ~= ccui.TouchEventType.ended then
                       return
                  end
              ...
              --  活动码
              local btn_VowTreeScene=pnl_Center:getChildByName("btn_VowTreeScene")
              btn_VowTreeScene:addTouchEventListener(function(sender, eventType  )
                 if eventType ~= ccui.TouchEventType.ended then
                       return
                  end
              ...
              --  惊喜吧
              local btn_ShopScene=pnl_Center:getChildByName("btn_ShopScene")
              btn_ShopScene:addTouchEventListener(function(sender, eventType  )
                 if eventType ~= ccui.TouchEventType.ended then
                       return
                  end
...
--  主界面小困件
function MainInterfaceScene:main_down_list(  )
      local pnl_ToolBar=self.MainScene_ToolBar:getChildByName("pnl_ToolBar")
      -- 列表控件
      local pnl_Extend=pnl_ToolBar:getChildByName("pnl_Extend")
      --  按钮控件
      local sp_More=pnl_ToolBar:getChildByName("sp_More")
      sp_More:addEventListener(function(sender, eventType  )
                     if eventType == ccui.CheckBoxEventType.selected then
                                pnl_Extend:setScale(0.0001)
                                -- local rotateTo = cc.RotateTo:create(1, 360)
                                -- pnl_Extend:runAction(rotateTo)
                     elseif eventType == ccui.CheckBoxEventType.unselected then
                                pnl_Extend:setScale(1)
                                -- local rotateTo = cc.RotateTo:create(1, 360)
                                -- pnl_Extend:runAction(rotateTo)
                     end
            end)
      --邮件
       local sp_Mail=pnl_ToolBar:getChildByName("sp_Mail")
       sp_Mail:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
       ...
       --好友
       local sp_Friends=pnl_ToolBar:getChildByName("sp_Friends")
       sp_Friends:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end   
       ...
       --邀请好友
       local sp_CarryFriendsNode=pnl_Extend:getChildByName("sp_CarryFriendsNode")
       sp_CarryFriendsNode:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
       ...
       --设置
       local sp_SetNode=pnl_Extend:getChildByName("sp_SetNode")
       sp_SetNode:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
       ...
       --任务
       local sp_TaskNode=pnl_Extend:getChildByName("sp_TaskNode")
       sp_TaskNode:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
       ...
       --签到
       local sp_SignNode=pnl_Extend:getChildByName("sp_SignNode")
       sp_SignNode:addTouchEventListener(function(sender, eventType  )
                         if eventType ~= ccui.TouchEventType.ended then
                             return
                        end
...
--用户数据
function MainInterfaceScene:fun_userdata(  )
        local userdt = LocalData:Instance():get_userdata()
       ...
       local head=self.pnl_PlayerStates:getChildByName("sp_PinleIcon")-- 头像
       if LocalData:Instance():get_user_head() then
            head:setTexture(LocalData:Instance():get_user_head())   --(tostring(Util:sub_str(userdt["imageUrl"], "/",":")))   ---
       end
      
       local name=self.pnl_PlayerStates:getChildByName("txt_PinleName")-- 名字
       local  userdata=LocalData:Instance():get_user_data()

        local nickname=userdata["loginname"]

        local nick_sub=userdata["nickname"]
       ...
       local crown_name=self.pnl_PlayerStates:getChildByName("txt_PinleHonour")-- 爵位
       crown_name:setString(userdt["rankname"])
       local leve=self.pnl_PlayerStates:getChildByName("txt_PinleLevel")-- 等级
       leve:setString("LV."  ..   userdt["grade"])
        local diamond=self.pnl_PlayerStates:getChildByName("txt_PinleDiamond")-- 钻石
       ...
       local gold_text=self.pnl_PlayerStates:getChildByName("txt_PinleGlod")-- 金币
       gold_text:setString(userdt["golds"])

       local ldb_PinleLevelBar=self.pnl_PlayerStates:getChildByName("ldb_PinleLevelBar")-- 进度条
       local jindu=tonumber(userdt["points"]) /  self.main_leve[tonumber(userdt["grade"])+1]  *  100 --self.main_leve[+1]/5000000 *100
       ldb_PinleLevelBar:setPercent(jindu)--self.main_leve
end
  ...
  NotificationCenter:Instance():AddObserver(G_NOTIFICATION_EVENT.USERINFOINIT_LAYER_IMAGE, self,
                       function()
                        ...
                        --  self:userdata()  --  等级刷新
                         ...
                         local getuserinfo=LocalData:Instance():get_getuserinfo()--保存数据
                         local gold_text=self.pnl_PlayerStates:getChildByName("txt_PinleGlod")
                        gold_text:setString(getuserinfo["golds"])
                         local userdt = LocalData:Instance():get_userdata()
                         userdt["golds"]=getuserinfo["golds"]
                         LocalData:Instance():set_userdata(userdt)
                         self:fun_userdata()  --  等级刷新
